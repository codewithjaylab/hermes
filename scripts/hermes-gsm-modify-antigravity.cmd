...
REM ── Check if Headroom proxy is running on port 8787 ────────────
netstat -ano | findstr "8787" >nul 2>&1
if !ERRORLEVEL! EQU 0 (
    echo [hermes-gsm] Headroom proxy detected on port 8787. Routing requests through proxy to save tokens.
    set "DEEPSEEK_BASE_URL=http://localhost:8787/v1"
    set "GEMINI_BASE_URL=http://localhost:8787/v1beta"
) else (
    echo [hermes-gsm] Headroom proxy not detected on port 8787. Direct connection will be used.
)
